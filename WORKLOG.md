...
- 공개 범위와 사실 교정: RTF에서 공개 제외한 학습 아카이브의 한·영 콘텐츠와 카탈로그 행을 현재
  트리에서 제거했다. 기존 공개 Git 이력은 자동 재작성하지 않았다. DDSC는 실제 provider가 제한된
  반응도 생성한다는 코드와 25개 현재 테스트에 맞춰 “채점만” 표현, 낡은 23개 수치와 확인되지 않은
  개인 기여 범위를 교정하고 글은 초안으로 유지했다.
...
- 독립 검수: 두 차례 diff review에서 인증 principal 소유 thread의 IDOR 보장 범위, DDSC provider
  출력 계약, PostGIS k6 threshold와 수동 EXPLAIN의 책임, 한·영 draft 상태, 생성기 원자성, 전체 경로
...
- 프로젝트: 공개 제외 대상인 비공개 학습 아카이브의 URL을 제거했다. con-dorm 표시명을 `Cham Domi (참 도미)`로 통일하되 기존 handle과 slug는 유지했다. Macro는 2024 해커톤 산출물로 교정하고 주문 전체 사전 검증, 실행 직렬화, 긴급 중단 latch, 수동 결제 경계와 테스트·CI를 반영했다.
...
- 사실 교정: ssu 플랫폼의 변동하는 합산 테스트·ADR 수치를 제거하고 네 번째 `ssu-ai-service` 저장소를 연결했다. con-dorm의 확인되지 않은 Docker Compose 설명을 제거했다. DDSC는 전체 완료가 아니라 스택 데모 완료·확장과 배포 작업 중인 프로토타입으로 변경하고 개인 역할을 스토리·정답 anchor·UI·아트로 한정했다. UNITHON은 사용자 커밋에서 확인되는 `macro_pkg` 20개 파일의 launcher·설정·패키징·통합만 직접 기여로 표시했다. 공개 제외 학습 아카이브와 기여 귀속 미확인 협업 프로젝트는 각각 보안 정리와 기여 귀속 확인 전까지 draft로 격리했다.
...
- 검증: 독립 리뷰에서 비공개 학습 아카이브 격리, UNITHON·DDSC 기여 범위, 그늘 지표·기록 설명과 coursework 언어 매핑을 교정했다. 11개 카테고리와 14개 프로젝트 관계·종류 검증, Astro 21개 파일 검사에서 오류·경고 0, 33개 생성 문서와 draft 격리, npm audit 0건을 확인했다. Git·CI·Production 전달은 다음 단계에서 진행한다.
