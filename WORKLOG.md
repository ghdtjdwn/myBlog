...
- 목표: draft PR #11과 Vercel Preview에 포함된 실제 ssuAI 챗봇 화면을 개인정보 관점에서 재검토하고,
  공개 포트폴리오에 불필요한 개인값을 즉시 제거한다.
- 발견: 학사 화면에는 개인 신원과 상세 이수 현황이, 도서관 화면에는 실제 좌석 식별자·위치·이용
  시간이 포함돼 있었다. draft와 `noindex`는 접근 제어가 아니므로 공개 가능한 증거로 볼 수 없다.
- 조치: 세 PNG와 한·영 프로젝트 갤러리 참조를 branch head에서 제거했다. 인증된 졸업요건 조회와
  HITL 예약 성공 사실은 개인값을 재노출하지 않는 텍스트 설명과 저장소의 ADR·테스트 기록으로만
  남긴다.
...
## 2026-07-17 — ssuAI 챗봇의 학사 조회와 승인 기반 좌석 예약 화면 추가

- 목표: ssuAI 자체 챗봇에서 인증된 졸업요건을 조회하고, 도서관 좌석 예약을 사용자 승인 전후로
  나누어 실행한 실제 화면을 프로젝트 README와 기술 블로그에서 같은 제품 근거로 확인할 수 있게 한다.
- ssuAI 문서: 한·영 README의 제품 화면 날짜를 2026-07-17로 갱신하고, 졸업요건 조회 화면과
  좌석 예약 승인 요청·완료 화면을 추가했다. 읽기 조회와 HITL 승인 기반 write 동작의 경계를
  실제 화면 순서에 맞춰 설명했다.
- 블로그: 한·영 ssu 플랫폼 프로젝트 갤러리에 ssuAI 챗봇 화면 3장과 대체 텍스트·설명을 추가해
  기존 대시보드 화면 및 외부 ChatGPT + ssuMCP 연동 화면과 구분했다.
- 공개 범위: 사용자가 공개를 요청한 원본 화면을 그대로 사용하되, 화면 속 개인 학점·좌석·세션
  세부 정보를 주변 설명에 반복하지 않는다. 한 번 성공한 실제 세션의 근거로만 설명하고 외부 학교
  시스템의 영구적 가용성을 일반화하지 않는다.
- 검증: ssuAI의 lint·typecheck·28개 테스트 파일 170개 테스트와 Next.js production build가
  통과했다. 블로그 `npm test`에서 11개 카테고리와 11개 한·영 프로젝트 관계, Astro 31개 파일
  진단 0건, 69개 생성 문서와 draft 격리가 통과했다. 로컬 한·영 ssu 플랫폼 상세는 각각 200이며
  대표 이미지와 갤러리 이미지 9개, 39개 반응형 이미지 응답을 확인했다.
- ssuAI 전달: commit `72c1e1f`을 PR #246으로 전달해 Node 20 CI·170개 테스트·gitleaks·Vercel
  Preview를 통과한 뒤 main에 fast-forward했다. main CI run `29565704298`과 Security run
  `29565704307`이 성공했고, 기본 README의 실제 GFM 렌더와 공개 raw PNG 3개의 200 응답을 확인했다.
- 검수 제한: 인앱 브라우저가 제공되지 않아 실제 데스크톱·모바일 viewport 캡처 검수는 수행하지
  못했다. 생성 HTML, 반응형 이미지 생성 결과와 로컬 HTTP 응답을 확인했다.
