...
- 사실 교정: ssu 플랫폼의 변동하는 합산 테스트·ADR 수치를 제거하고 네 번째 `ssu-ai-service` 저장소를 연결했다. con-dorm의 확인되지 않은 Docker Compose 설명을 제거했다. DDSC는 전체 완료가 아니라 스택 데모 완료·확장과 배포 작업 중인 프로토타입으로 변경하고 개인 역할을 스토리·정답 anchor·UI·아트로 한정했다. UNITHON은 사용자 커밋에서 확인되는 `macro_pkg` 21개 파일의 launcher·설정·패키징·통합만 직접 기여로 표시했다. firstStudy와 기여 귀속 미확인 협업 프로젝트는 각각 보안 정리와 기여 귀속 확인 전까지 draft로 격리했다.
