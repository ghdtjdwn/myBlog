...
- 변경: 스택 ADR의 선택 근거를 동적 기능 필요성, 장애 도메인, 지속 비용과 운영 책임으로 다시 적고, Vercel alias 장애 기록에서 인터뷰 질문 목록을 삭제했다. 애플리케이션 코드와 콘텐츠 route는 바꾸지 않았다.
- 결정: 포트폴리오 겸 기술 블로그라는 공개 정체성과 과거 전달 기록은 사실이므로 보존한다. 제거 범위는 기술 결정을 평가 효과로 정당화하는 표현과 장애 문서의 인터뷰용 질문으로 제한한다.
