...
프로젝트 기록과 기술 글을 한 도메인에서 운영하고, 여러 프로젝트의 ADR·작업 로그·장애 기록을 빠르게 공개 초안으로 전환해야 한다. 동시에 블로그 자체의 운영 복잡성이 글 작성과 공모전·팀 활동을 방해하면 안 된다.

다른 두 서비스가 이미 별도의 운영 인프라를 사용한다.
...
따라서 동적 기능이 필요하지 않은 블로그에서 같은 인프라를 반복하면 운영 표면과 장애 의존성만 늘어난다.
...
기각. 블로그에는 서버 컴포넌트나 상시 API가 필요하지 않다. 콘텐츠 사이트에는 Astro가 더 작은 런타임과 단순한 빌드·배포 경로를 제공한다.

### AWS S3 + CloudFront + Route 53 + Terraform

기각. 안전한 정적 사이트를 만들 수 있지만 Preview 환경과 글 전달을 위해 추가 구성이 필요하고, 소액이라도 지속 비용과 운영 표면이 생긴다.

### 기존 Oracle k3s

기각. ssu 플랫폼과 장애 도메인을 공유하게 되고 개인 블로그의 가용성이 단일 노드 운영에 종속된다. 독립 배포에 비해 결합도와 복구 책임이 커진다.
...
- 동적 기능이 없는 공개 페이지에는 관리형 정적 배포가 요구사항과 운영 비용에 가장 잘 맞는다.
