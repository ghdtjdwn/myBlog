...
V1에서는 DB, CMS, Kubernetes, 서버리스 API, Terraform을 사용하지 않는다. `ssuAI` 플랫폼에서 k3s·ArgoCD·관측성을, `그늘`에서 AWS ECS·Terraform·RDS·CloudFront를 이미 훨씬 깊게 증명했기 때문이다. 블로그는 전달 속도와 콘텐츠 안정성이 우선이다.
...
2. 정적 빌드
3. 내부 링크, 이미지 경로, canonical, JSON-LD, RSS, sitemap 검사
...
- `scripts/new-record.mjs`: project/post/decision/incident 초안 생성
...
자동화는 글을 발행하지 않는다. 프로젝트의 work log, ADR, troubleshooting, Git diff를 읽어 초안을 만들 수 있지만 `draft: true`를 강제한다. 비공개 원문은 공개 저장소에 복사하지 않는다.

## 9. V1 이후 조건부 확장

- 글 20편 이상: Pagefind 정적 검색 검토
- 독자 피드백 수요 확인: Giscus 검토
- 모바일 작성이 실제 병목: Headless CMS 검토
- 상호작용이 필요한 글: 해당 MDX 컴포넌트만 island로 추가
- 영문 수요 확인: 대표 글부터 선택적 번역
