...
PR #6 merge commit `346cfff`와 연결된 Production
`dpl_2TNKJhzXKHqMPwkik8YmbAtEQT9C`가 Ready임을 확인한 뒤 본진 alias를 승격했다. 다음 조건을
실제 `seongju.vercel.app`에서 모두 확인했다.
...
- 변경 전 deployment URL과 ID를 rollback 값으로 보존
