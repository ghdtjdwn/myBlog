    ...
    site: singleton({
      ...
      schema: {
        ...
        home: fields.object({
          ...
          projectsLabelKo: fields.text({ label: "프로젝트 목록 제목 (한국어)", validation: { isRequired: true } }),
          projectsLabelEn: fields.text({ label: "Projects list title (English)", validation: { isRequired: true } }),
          recentPostCount: fields.integer({ label: "홈에 표시할 최근 글 수", defaultValue: 6 }),
          projectCount: fields.integer({ label: "홈에 표시할 프로젝트 수", defaultValue: 3 }),
        }, { label: "홈 화면" }),
  ...
  collections: {
    ...
    categories: collection({
      ...
      schema: {
        ...
        kind: fields.select({ label: "분류 종류", options: [
          { label: "기술 주제", value: "topic" }, { label: "활동 유형", value: "activity" },
        ], defaultValue: "topic" }),
    ...
    projects: collection({
      ...
      schema: {
        ...
        image: fields.image({ label: "커버 이미지", directory: "src/assets/projects", publicPath: "../../assets/projects/" }),
